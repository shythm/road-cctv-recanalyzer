...
def interpolate_persp_data(persp_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # join tracking data
    df = df.join(persp_df.set_index(['objid', 'frame']), on=['objid', 'frame'], how='left')
    ...
    df['x'] = df.groupby('objid')['x'].apply(
        lambda x: x.interpolate(method='linear')).reset_index(drop=True)
    df['y'] = df.groupby('objid')['y'].apply(
        lambda x: x.interpolate(method='linear')).reset_index(drop=True)

    # interpolate missing pos_x, pox_y with linear interpolation
    df['perspx'] = df.groupby('objid')['perspx'].apply(
        lambda x: x.interpolate(method='linear')).reset_index(drop=True)
    df['perspy'] = df.groupby('objid')['perspy'].apply(
        lambda x: x.interpolate(method='linear')).reset_index(drop=True)
...
class CCTVTrackingAnalysisTaskSrv(TaskService):
    ...
    def __init__(
            self, task_repo: TaskItemRepository,
    ...
    def get_params(self) -> list[dict]:
        return [
            ...
            TaskParamMeta(
                name="roi", desc="ROI 좌표", accept=["json"]),
            TaskParamMeta(
                name="roadwidth", desc="도로 너비(m)", accept=["float"]),
            TaskParamMeta(
                name="roadheight", desc="도로 길이(m)", accept=["float"]),
        ]
        ...
        def task_func():
            ...
            try:
                ...
                matrix = cv2.getPerspectiveTransform(
                    np.array(srcpoints, dtype=np.float32), np.array(dstpoints, dtype=np.float32))
                ...
                df['_dst'] = df['_src'].apply(
                    lambda p: cv2.perspectiveTransform(np.array([[p]], dtype=np.float32), matrix)[0][0])
                ...
                for objid in df['objid'].unique():
                    ...
                    spds = spds.diff(periods=delta_frame)  # it assume that data is sorted by frame
                ...
                cap = cv2.VideoCapture(os.path.join(self._outputs_path, task.params["targetname"]))
                cap_size = (roiwidth, roiheight)
                fourcc = cv2.VideoWriter.fourcc(*'mp4v')
                cap_out = cv2.VideoWriter(
                    os.path.join(self._outputs_path, f"{task.id}.mp4"), fourcc, fps, cap_size)
